...
import 'package:kindy/screens/events/sheets/add_event_sheet.dart';
...
class _CalendarScreenState extends State<CalendarScreen> {
  ...
  Widget _buildEventCard(Event e, PigioThemeData theme, PigioAppState state) {
    ...
    // Build subtitle with age for birthdays
    String subtitle;
    if (e.typeEn == 'Birthday' && linkedContact != null) {
      final age = linkedContact.age;
      if (age != null) {
        final nextAge = age + (e.daysRemaining > 0 ? 1 : 0);
        subtitle = linkName != null ? "$linkName · Tourne $nextAge ans" : "Tourne $nextAge ans";
      } else {
        subtitle = linkName != null ? "$linkName · ${e.typeFr}" : e.typeFr;
      }
    } else if (e.isRecurring && e.age != null && e.age! > 0) {
      final suffix = "${e.age!} ans";
      subtitle = linkName != null ? "$linkName · $suffix" : suffix;
    } else {
      subtitle = linkName != null ? "$linkName · ${e.typeFr}" : e.typeFr;
    }

    // Auto-generated birthday events are not deletable/editable
    final isAutoGenerated = e.id.startsWith('birthday_');
    ...
    Widget card = GestureDetector(
      onTap: () {
        if (isAutoGenerated) {
          ...
          // Open edit sheet for user-created events
          showAddEventSheet(context, existingEvent: e, onAdded: () => setState(() {}));
        }
      },
      child: Container(
    ...
    // Wrap user-created events in Dismissible for swipe-to-delete
    if (!isAutoGenerated) {
      card = Dismissible(
        key: ValueKey(e.id),
        ...
        confirmDismiss: (_) async {
          return await showDialog<bool>(
            ...
            builder: (ctx) => AlertDialog(
              ...
              title: Text("Supprimer ?", style: fw(size: 18, w: FontWeight.w900, color: theme.ink)),
              content: Text("Supprimer « ${e.title} » ?", style: fw(size: 14, color: theme.mid)),
              actions: [
                TextButton(onPressed: () => Navigator.pop(ctx, false), child: Text("Annuler", style: fw(size: 14, w: FontWeight.w700, color: theme.mid))),
                TextButton(onPressed: () => Navigator.pop(ctx, true), child: Text("Supprimer", style: fw(size: 14, w: FontWeight.w700, color: theme.error))),
              ],
            ),
          );
        },
